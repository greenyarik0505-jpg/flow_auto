...
## 👥 Мультиаккаунтинг (40+ профилей Chrome без повторного входа)

Если у вас (или ваших коллег) настроено **40 аккаунтов в Google Chrome**, вам **НЕ НУЖНО заново входить в каждый аккаунт**!
...
### 2. Запуск Chrome в режиме автоматизации (CDP):
```powershell
.\start_chrome_debug.bat
```
Этот батник открывает ваш привычный Google Chrome со всеми вашими 40 профилями и разрешает локальное подключение генератора Flow через порт `9222`. После этого генератор может использовать любую открытую сессию **мгновенно и без ре-логина**.
...
├── list_profiles.py        # Скрипт сканирования профилей Google Chrome
├── start_chrome_debug.bat  # Запуск Chrome с портом 9222 (Zero-Login для всех 40 аккаунтов)
├── capture_session.bat     # Быстрый захват сессии из браузера в 1 клик
├── capture_session.py      # Скрипт сохранения состояния сессии
├── login.bat               # Батник быстрой проверки и привязки профилей
├── login.py                # Скрипт проверки сессий и аккаунтов
