...
### 2. Автоматический захват куков по очереди (для всех 40 профилей):
Запустите привязку всех аккаунтов одной командой:
```powershell
.\login.bat --all
```
* Скрипт по очереди открывает каждый ваш профиль в настоящем Chrome (со всеми аккаунтами, **без режима гостя**!).
* Как только страница Google Flow загружается:
  - **Автоматически:** расширение `flow_extension` само передает куки в скрипт за 0.1 секунды!
  - **Либо в 1 клик:** нажмите на значок расширения в Chrome ➔ *«Скопировать и отправить»*.
  - **Либо через Ctrl+C:** скопируйте токен в буфер обмена (из F12 ➔ Application ➔ Cookies ➔ `__Secure-next-auth.session-token`) — скрипт сам перехватит его из буфера!
* Скрипт **сразу сохраняет сессию, валидирует её через API и МГНОВЕННО открывает следующий профиль**!
