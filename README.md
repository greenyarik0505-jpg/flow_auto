...
1. На экране откроется реальное окно **Google Chrome**.
2. Войдите в ваш аккаунт Google, на котором подключен доступ к Google Flow.
3. Дождитесь, пока загрузится интерфейс самой студии (поле ввода промпта).
4. **Закройте окно Chrome крестиком**.

Сессия сохранится в профиле `%LOCALAPPDATA%\ffroliva\gflow-cli\profile_default\` и будет действовать неделями без повторных входов.

Проверить статус сессии в любой момент:
```powershell
.\.venv\Scripts\gflow.exe auth status
```
