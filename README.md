...
### 2. Сохранение сессии для профиля (без режима гостя!):
Запустите привязку нужного профиля (например, профиль 2):
```powershell
.\login.bat 2
```
* Скрипт открывает вкладку Google Flow в вашем **НАСТОЯЩЕМ Chrome под выбранным профилем** (со всеми вашими сохраненными аккаунтами, без режима гостя и без блокировок Google!).
* Скопируйте токен авторизации (`__Secure-next-auth.session-token`) через расширение `flow_extension` (в 1 клик) или через `F12 -> Application -> Cookies`.
* Вставьте токен в консоль — сессия сохранится навсегда, и генератор сможет работать полностью в фоне без открытых окон!
* Также можно передать токен сразу одной командой:
  ```powershell
