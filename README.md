...
[![Google Flow](https://img.shields.io/badge/Google%20Flow-Veo%20%7C%20Gemini%20Omni%20%7C%20Imagen-red.svg)](https://flow.google.com/)

Автоматизированный инструмент и локальный **REST Web2API сервер** для генерации видео и изображений через студию **Google Flow** ([flow.google.com](https://flow.google.com/)).
...
4. [Мультиаккаунтинг и ротация](#-мультиаккаунтинг-40-профилей-chrome)
5. [Генерация Видео через Терминал (CLI)](#-1-генерация-видео-через-терминал-cli)
6. [Генерация Фото через Терминал (CLI)](#-2-генерация-фото-через-терминал-cli)
7. [Web2API (REST API & Swagger UI)](#-3-web2api-rest-api--swagger-ui)
8. [Примеры интеграции (Python / cURL / Telegram Bot)](#-4-примеры-интеграции)
9. [Таблица поддерживаемых моделей](#-5-таблица-моделей-и-форматов)
10. [Структура проекта](#-6-структура-проекта)
11. [Частые вопросы и Решение проблем](#-7-частые-вопросы-и-faq)
...
- ⚡ **Новейший движок Google Flow Studio**: прямая интеграция с `flow.google.com` (Angular-приложение Google AI).
- 🧠 **Мгновенный захват сессий из RAM**: считывает авторизационные куки (`OSID`, `SID`, `__Secure-next-auth`) напрямую из сетевой службы Chrome за доли секунды. **Без расширений, без DevTools и без перезапуска браузера!**
- 🎬 **Полный контроль параметров**: поддержка соотношений сторон `16:9`, `9:16` (Shorts/Reels), `1:1`, `4:3`, `3:4`, длительности клипов до 10 секунд и пакетной генерации.
- 🔄 **Авто-ротация 40+ профилей**: умное переключение на следующий аккаунт при исчерпании лимитов или квот (`--profile auto`).
...
└──────────────────────────┬─────────────────────────────┘
    ...
    [ generate.bat / CLI ]        [ server.py / REST API ]
            ...
            └──────────────┬──────────────┘
                           ▼
          flow_engine.py (Direct Studio Automation)
                           │
                           ▼
            Playwright Chromium (Headless)
                           │
                           ▼
          Google Flow Studio (flow.google.com)
...
### Проверка готовых профилей:
```powershell
.\login.bat --list
```
Команда обратится к Google API и покажет все ваши авторизованные профили Chrome и их статус готовности.
...
### 2. Автоматический захват сессий по очереди (для всех профилей):
Запустите привязку всех аккаунтов одной командой:
```powershell
.\login.bat --all
```
* Скрипт по очереди открывает каждый ваш профиль в настоящем Google Chrome (`flow.google.com`).
* **Никаких расширений и DevTools:** скрипт моментально извлекает куки Google SSO из оперативной памяти Chrome (Network Service RAM) без перезапуска браузера.
* **Автопроверка через API:** скрипт сразу проверяет профиль через официальный эндпоинт сессии Google.
* **Мгновенный переход:** сессия сохраняется в `.flow_sessions/`, и скрипт сразу переходит к следующему профилю!
...
├── .flow_sessions/         # Сохраненные сессии и куки аккаунтов Chrome
├── output/                 # Папка с сгенерированными видео и фото
│   ├── videos/             # Готовые .mp4 клипы
│   └── images/             # Готовые .png фото
├── flow_engine.py          # Автономный движок студии Google Flow (Playwright)
...
├── login.py                # Скрипт захвата сессий из памяти Chrome и проверки API
